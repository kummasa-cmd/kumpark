...
export const metadata: Metadata = { title: "전자책 코칭 신청" };
export const dynamic = "force-dynamic";

export default async function ApplyEbookCoachingPage() {
  ...
  const duration = getCoachingDuration("ebook", settings);
  const amount = getEffectivePrice("ebook", settings);
  ...
  return (
    <div className="space-y-5 max-w-2xl">
      <div>
        ...
        >
          <ChevronLeft size={15} /> 전자책 코칭 소개
        </Link>
        <h1 className="text-xl font-bold text-gray-900">전자책 코칭 신청</h1>
        <p className="text-sm text-gray-500 mt-0.5">검전쓰 1대1 전자책 코칭을 신청합니다.</p>
      </div>
      <CoachingApplyForm
        bookType="ebook"
        productName="전자책 코칭"
        ...
        blocked={blocked}
        blockedMessage="이미 진행중인 전자책 코칭이 있어 신청할 수 없습니다."
      />
    </div>
  );
}
